...
### Connection Draining
CLBだけ。登録解除または異常なインスタンスに対して行われた実行中のリクエストを完了することが可能。  
ALBはこの機能ではなく、単純にターゲットグループからターゲットを登録解除すると、同様の挙動になる。ターゲットは未完了のリクエストが完了するまで、draining状態になる。

### スティッキーセッション
リクエストにある特定のCoockieから同じユーザから来たと判別して特定のEC2インスタンスに送信する。デフォルト無効。
