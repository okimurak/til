...
- S3 Standard (STANDARD) ... 複数 AZ にデータを複製 (耐久性 99.9999999999 ％ の SLA)
  - 頻繁にアクセスされるデータ
- S3 Standard–IA (STANDARD_IA) (標準低頻度アクセスストレージ) ... 複数 AZ にデータを複製
  - プライマリ or 再作成できないデータのコピーでのみ使用
- S3 Intelligent-Tiering (INTELLIGENT_TIERING) ... 低頻度アクセス層と高頻度アクセス層を自動で行き来
  - 未知のアクセスパターンのデータに使える
  - 30 日以内にアクセスが有ると STANDARD
  - 30 日アクセスがないと STANDARD_IA に移行
  - 90 日アクセスがないと GLACIER_IR に移行
- [S3 Express One Zone (EXPRESS_ONEZONE)](https://docs.aws.amazon.com/ja_jp/AmazonS3/latest/userguide/directory-bucket-high-performance.html#s3-express-one-zone) ... アクセス頻度が高く、低レイテンシー向け。
- S3 One Zone-IA (ONEZONE_IA) ... 1 つの AZ に格納。
  - データが再作成できる場合や、クロスリージョンレプリケーション(CRR)設定時のオブジェクトレプリカで使用
- S3 Glacier Instant Retrieval(GLACIER_IR) ... アクセス頻度は低く、長期保存(90 日以上) を想定しているが、すぐに取り出したいときにつかう。
- S3 Glacier Flexible Retrieval (GLACIER)... 安価なストレージ。90 日の保存必要。取り出しに時間かかる
- S3 Glacier Deep Archive(DEEP_ARCHIVE) ... Glacier よりも安価。取り出しにとても時間かかる(12 時間~48 時間)
...
[アクセスログ](https://docs.aws.amazon.com/ja_jp/AmazonS3/latest/userguide/ServerLogs.html#LogDeliveryBestEffort)はベストエフォートな点に注意。
