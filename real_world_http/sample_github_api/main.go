...
var clientID = "<github oauth2 clinet id>"
var clientSecret = "<github oauth2 clinet secret>"
	...
	if os.IsNotExist(err) {
		...
		server = &http.Server{
			...
			Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				io.WriteString(w, "<html><script>window/open('abount:blank', '_self').close()</script></html>")
...
func putGist(client *http.Client) {
	gist := `{
